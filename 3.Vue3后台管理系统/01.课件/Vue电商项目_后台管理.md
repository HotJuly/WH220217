...
1. 权限管理由2部分组成
	...
	需求:根据帐号的权限信息,控制用户能够访问的路由模块
		思路:如果用户不具备访问A路由的权限,我就不注册这个路由,用户无论如何都进不去
		拆解:
			1.如何知道一个账号具有哪些权限?
				通过请求服务器可以知道,接口是info接口
				通过返回的routes数组,可以知道用户能访问哪些异步路由
            2.如何控制用户能够访问哪些路由?
            	将原本静态注入的所有的路由对象划分为3种
            		1.在项目初始化的时候,只注入常量路由
            		2.根据返回的routes数组,来实现动态注入部分异步路由的效果
            			语法:router.addRoute(路由对象)
                问题:根据route数组生成一个用户能够访问的异步路由组成的数组,来实现动态注入
	
	我们实现的权限控制是基于后台权限相关数据的实现
	
2. 有2个后台用户
	admin/111111: 超级管理员 ==> 有所有路由和按钮的权限
	wh220217a/111111: 普通用户 ==> 只有部分路由和按钮的权限
...
6. 用户权限相关数据结构?
	roles: ["一般管理员", "aaa"]->说明当前账号具有哪些角色信息,暂时没用
	routes: ["RoleAuth", "Role", "User"]->说明当前账号能够访问哪些路由组件,用于实现路由级权限管理
	buttons: ["btn.User.add", "btn.Role.assgin"]->说明当前账号能够使用哪些按钮的,用于实现按钮级权限管理
	
7. 注册路由的2种方式
	静态注册: 项目初化时, 创建路由器时通过routes配置注册上   ==> Login/DashBoard
		routes: [路由1, 路由2]
	动态注册: 用户登陆后获取得到用户的权限数据后才去
		router.addRoutes([路由3, 路由4])->这个已经废弃了
		router.addRoute(路由1)->最新语法
